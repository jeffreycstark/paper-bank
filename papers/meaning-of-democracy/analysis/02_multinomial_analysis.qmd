...
# Parallel processing for bootstrap (optional but recommended)
# If not installed: install.packages(c("future", "future.apply"))
if (requireNamespace("future", quietly = TRUE) &&
    requireNamespace("future.apply", quietly = TRUE)) {
  library(future)
  library(future.apply)
  # Use all available cores minus 1 for parallel bootstrap
  plan(multisession, workers = max(1, parallel::detectCores() - 1))
  cat("Parallel processing enabled with", max(1, parallel::detectCores() - 1), "workers\n")
} else {
  cat("Parallel processing not available (install future and future.apply for faster bootstrap)\n")
}
...
#
# Uses parallel processing via future.apply when available for faster execution.
# Reports number of successful iterations to detect potential issues with
# bootstrap sample variation (e.g., categories with zero observations).
bootstrap_ames <- function(model_formula, data, n_boot = 500, seed = 42, cluster_var = "country_name",
  ...
  # Define single bootstrap iteration function
  run_single_boot <- function(i, clusters, n_clusters, data, cluster_var, model_formula) {
  ...
  use_parallel <- parallel && requireNamespace("future.apply", quietly = TRUE)

  if (use_parallel) {
    # Parallel bootstrap using future.apply
    # Note: User should set up future plan before calling (e.g., plan(multisession))
    if (!future::plan() %in% c("sequential")) {
      ...
      # Fall back to sequential if no parallel plan set
      boot_results <- lapply(
        1:n_boot,
        run_single_boot,
        clusters = clusters,
  ...
  # Convert results to matrix and count successes
  boot_ames <- matrix(NA, nrow = n_boot, ncol = length(choice_levels))
  colnames(boot_ames) <- choice_levels
  n_success <- 0

  for (i in 1:n_boot) {
    if (!is.null(boot_results[[i]])) {
      boot_ames[i, ] <- boot_results[[i]]
  ...
  if (verbose) {
    cat(sprintf("  Bootstrap: %d/%d iterations succeeded (%.1f%%)\n",
                n_success, n_boot, success_rate))
    if (success_rate < 90) {
      cat("  ⚠️ Warning: Low success rate may indicate model instability or sparse categories\n")
    }
  }

  # Calculate SEs (only if we have enough successful iterations)
  if (n_success < n_boot * 0.5) {
    warning(sprintf("Only %d of %d bootstrap iterations succeeded. SEs may be unreliable.",
                    n_success, n_boot))
  }

  ses <- apply(boot_ames, 2, sd, na.rm = TRUE)

  # Return SEs with success count as attribute
  attr(ses, "n_success") <- n_success
  attr(ses, "n_boot") <- n_boot
