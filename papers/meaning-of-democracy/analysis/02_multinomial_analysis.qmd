...
# Robustness: Pooled Models Without Demographic Controls

This section reruns the same pooled multinomial logits without demographic controls
to verify that the loser effect is not driven by compositional differences between
winner and loser populations.
...
results_list <- list(
  ...
  # Pooled results (without demographic controls - robustness)
  all_results_no_controls = all_results_nc,
  controls_comparison = comparison,
...
# Save no-controls results for robustness comparison
write_csv(all_results_nc, file.path(results_dir, "loser_effects_all_items_no_controls.csv"))
cat("CSV saved to:", file.path(results_dir, "loser_effects_all_items_no_controls.csv"), "\n")
...
cat("1. loser_effects_all_items.csv - Pooled AMEs for all 20 items (with controls)\n")
cat("2. loser_effects_all_items_no_controls.csv - Pooled AMEs without demographic controls\n")
cat("3. country_wave_item_ames.csv - Item-level AMEs by country-wave\n")
cat("4. country_wave_type_summary.csv - Type-level summary by country-wave\n")
cat("5. country_wave_proc_sub_gap.csv - Procedural-Substantive gap by country-wave\n")
cat("6. fig_loser_effects_all_items.pdf - Coefficient plot (pooled)\n")
cat("7. fig_country_trajectories.pdf - Country trajectory plot\n")
```
