...
if (!is.null(cw_ames)) {
  ...
  cw_display <- cw_ames %>%
    ...
    select(country_name, wave_label, set, item, ame_fmt) %>%
    pivot_wider(
      names_from = c(set, item),
      values_from = ame_fmt,
      names_sep = ": "
    ) %>%
    arrange(country_name, wave_label)
  ...
  cw_display %>%
    ...
    kable(booktabs = TRUE, longtable = TRUE, font_size = 7) %>%
...
if (!is.null(cw_gap)) {
  ...
  ggplot(multi_wave, aes(x = wave, y = proc_sub_gap * 100)) +
    ...
    labs(
      ...
      title = "Country-Level Trajectories of the Loser Effect",
