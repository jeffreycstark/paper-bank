...
library(tidyverse)
library(broom)
library(sandwich)   # HC2 robust SEs (no PSU/strata in ABS harmonized data)
library(lmtest)
...
## Wave-by-wave OLS — Econ → Satisfaction vs. Quality vs. Auth Rejection {#sec-wave-ols}

H1, H2, H3: Does economic voting predict satisfaction, quality, and/or auth
rejection differently by country and wave?
...
for (cntry in c("Korea", "Taiwan")) {
  for (w in 1:6) {
    ...
    f_auth <- as.formula(paste("auth_reject_index ~ econ_index +", controls))
    m_auth <- lm(f_auth, data = sub)
    ...
    wave_results[[paste(cntry, w, sep = "_")]] <- bind_rows(
      ...
      extract_econ_coef(m_qual) |> mutate(dv = "Democratic quality", country = cntry,
                                           ...
                                           n = nobs(m_qual)),
      extract_econ_coef(m_auth) |> mutate(dv = "Auth rejection", country = cntry,
                                           wave = w, r_sq = summary(m_auth)$r.squared,
                                           n = nobs(m_auth))
    )
  }
}
...
cat("\n--- Korea: Auth rejection ---\n")
wave_df |>
  filter(country == "Korea", dv == "Auth rejection") |>
  mutate(sig = sig_stars(p.value)) |>
  dplyr::select(wave, estimate, std.error, p.value, sig, n) |>
  print()
```
...
for (cntry in c("Korea", "Taiwan")) {
  ...
  f_auth_int <- as.formula(paste("auth_reject_index ~ econ_index * wave_c +", controls))
  m_auth_int <- lm(f_auth_int, data = sub)

  sat_tidy  <- tidy_hc2(m_sat_int) |>
  ...
  qual_tidy <- tidy_hc2(m_qual_int) |>
    filter(term %in% c("econ_index", "econ_index:wave_c")) |>
    mutate(dv = "Democratic quality", country = cntry,
           r_sq = summary(m_qual_int)$r.squared, n = nobs(m_qual_int))

  auth_tidy <- tidy_hc2(m_auth_int) |>
    filter(term %in% c("econ_index", "econ_index:wave_c")) |>
    mutate(dv = "Auth rejection", country = cntry,
           r_sq = summary(m_auth_int)$r.squared, n = nobs(m_auth_int))

  pooled_results[[cntry]] <- bind_rows(sat_tidy, qual_tidy, auth_tidy)

  cat(cntry, ":\n")
  bind_rows(sat_tidy, qual_tidy, auth_tidy) |>
...
Is the Korea–Taiwan divergence in the econ → quality and econ → auth rejection
...
f_xc_auth <- as.formula(paste("auth_reject_index ~ econ_index * is_korea + factor(wave) +", controls))
m_xc_auth <- lm(f_xc_auth, data = both |> filter(!is.na(auth_reject_index)))

xc_sat_tidy <- tidy_hc2(m_xc_sat) |>
  filter(term %in% c("econ_index", "is_korea", "econ_index:is_korea")) |>
  mutate(dv = "Satisfaction")

xc_qual_tidy <- tidy_hc2(m_xc_qual) |>
  filter(term %in% c("econ_index", "is_korea", "econ_index:is_korea")) |>
  mutate(dv = "Democratic quality")

xc_auth_tidy <- tidy_hc2(m_xc_auth) |>
  filter(term %in% c("econ_index", "is_korea", "econ_index:is_korea")) |>
  mutate(dv = "Auth rejection")

xc_df <- bind_rows(xc_sat_tidy, xc_qual_tidy, xc_auth_tidy)
...
kr_auth_effect <- xc_auth_tidy$estimate[xc_auth_tidy$term == "econ_index"] +
                  xc_auth_tidy$estimate[xc_auth_tidy$term == "econ_index:is_korea"]

cat(sprintf("\nKorea econ -> satisfaction    = %.3f\n", kr_sat_effect))
cat(sprintf("Korea econ -> quality         = %.3f\n", kr_qual_effect))
cat(sprintf("Korea econ -> auth rejection  = %.3f\n", kr_auth_effect))
...
cat(sprintf("Taiwan econ -> auth rejection = %.3f\n",
            xc_auth_tidy$estimate[xc_auth_tidy$term == "econ_index"]))
```
...
### App B: Four-bucket disaggregated DVs (both countries) {#sec-app-b}

```{r app-b-four-buckets}
...
bucket3_vars <- c("sys_proud_n", "sys_prefer_n", "sys_capable_n",
                  "qual_sys_support_n", "qual_sys_change_n")
# Bucket 4: Auth rejection
bucket4_vars <- c("strongman_reject_n", "military_reject_n",
                  "expert_reject_n", "singleparty_reject_n",
                  "auth_reject_index")

all_dv_vars <- c(bucket1_vars, bucket2_vars, bucket3_vars, bucket4_vars)
  ...
  for (w in 1:6) {
    ...
    for (dv in all_dv_vars) {
      ...
      bucket <- case_when(
        ...
        dv %in% bucket4_vars ~ "Auth rejection"
      )
...
for (cntry in c("Korea", "Taiwan")) {
  ...
  for (dv_info in list(
    ...
    ologit_results[[paste(cntry, dv_info[1], sep = "_")]] <- tibble(
      ...
      p.value   = 2 * pt(abs(econ_row["t value"]), df = nobs(m) - length(coef(m)),
                          lower.tail = FALSE),
      n         = nobs(m)
    )
  }
...
for (cntry in c("Korea", "Taiwan")) {
  for (w in 1:6) {
    ...
    # Use weight variable if available; fall back to equal weights
...
```{r app-f-identity}
# F.1: National pride x Econ -> Quality (both countries)
...
# F.2: China threat x Econ -> Quality (Taiwan only, W3-6)
china_results <- list()
...
## Appendix-compatible model results {#sec-appendix-compat}

Compute the five-component structure consumed by `skdr-online-appendix.qmd`.
All models use HC2 robust SEs via the updated `extract_econ_coef` / `tidy_hc2` helpers.

```{r appendix-models}
# Derive winner_loser if not already present (voted_winning_losing: 1=Winner, 2=Loser)
...
# DV label mapping (5 individual items)
dv_labels <- c(
...
# ── A: Wave-by-wave (5 individual DVs, both countries) ──────────────────────
wbw_app <- list()
for (cntry in c("Korea", "Taiwan")) {
  for (w in 1:6) {
    sub <- dat |> filter(country_label == cntry, wave == w)
    ...
    for (dv in names(dv_labels)) {
      ...
      m <- tryCatch(lm(f, data = sub), error = function(e) NULL)
      if (is.null(m)) next
      wbw_app[[paste(cntry, w, dv, sep = "_")]] <-
        extract_econ_coef(m) |>
        mutate(dv = dv, dv_label = dv_labels[dv],
               country = cntry, wave = w,
               r_sq = summary(m)$r.squared, n = nobs(m))
    }
  }
}
wave_by_wave_app <- bind_rows(wbw_app)

cat("wave_by_wave_app rows:", nrow(wave_by_wave_app), "\n")
cat("dv_labels:", unique(wave_by_wave_app$dv_label), "\n")

# ── B: Pooled individual-DV models (5 DVs × 2 countries, wave FE) ──────────
pooled_app <- list()
for (cntry in c("Korea", "Taiwan")) {
  sub <- dat |> filter(country_label == cntry)
  for (dv in names(dv_labels)) {
    ...
    m <- tryCatch(lm(f, data = sub), error = function(e) NULL)
    if (is.null(m)) next
    pooled_app[[paste(cntry, dv, sep = "_")]] <-
      extract_econ_coef(m) |>
      mutate(dv = dv, dv_label = dv_labels[dv],
             country = cntry, r_sq = summary(m)$r.squared, n = nobs(m))
  }
}
pooled_indiv_app <- bind_rows(pooled_app)

cat("\npooled_indiv_app rows:", nrow(pooled_indiv_app), "\n")

# ── C: Cross-country interaction (5 DVs) ────────────────────────────────────
xc_app <- list()
both <- dat |> mutate(is_korea = as.numeric(country_label == "Korea"))
for (dv in names(dv_labels)) {
  ...
  m <- tryCatch(lm(f, data = sub), error = function(e) NULL)
  if (is.null(m)) next
  xc_app[[dv]] <- tidy_hc2(m) |>
    filter(term %in% c("econ_index", "is_korea", "econ_index:is_korea")) |>
    mutate(dv = dv, dv_label = dv_labels[dv], n = nobs(m))
}
cross_country_app <- bind_rows(xc_app)

cat("\ncross_country_app rows:", nrow(cross_country_app), "\n")

# ── D: Political interest subgroups (Korea, 3 DVs) ──────────────────────────
polint_dvs <- c(qual_pref_dem_n   = "Dem always preferable",
                sat_democracy_n   = "Satisfaction with democracy",
                auth_reject_index = "Auth rejection index")
...
for (grp in c("High interest", "Low interest")) {
  ...
  if (nrow(sub) < 200) next
  for (dv in names(polint_dvs)) {
    if (all(is.na(sub[[dv]]))) next
    f <- as.formula(paste(dv, "~ econ_index + factor(wave) +", controls))
    m <- tryCatch(lm(f, data = sub), error = function(e) NULL)
    if (is.null(m)) next
    polint_app[[paste(grp, dv, sep = "_")]] <-
      extract_econ_coef(m) |>
      mutate(dv = dv, dv_label = polint_dvs[dv],
             country = "Korea", group = grp, n = nobs(m))
  }
}
polint_subgroup_app <- bind_rows(polint_app)

cat("\npolint_subgroup_app rows:", nrow(polint_subgroup_app), "\n")

# ── E: Winner–loser subgroups (both countries, 2 DVs) ───────────────────────
winner_dvs <- c(qual_pref_dem_n = "Dem always preferable",
                sat_democracy_n = "Satisfaction with democracy")
...
for (cntry in c("Korea", "Taiwan")) {
  ...
  for (grp in c("Winner", "Loser")) {
    ...
    if (nrow(sub) < 100) next
    for (dv in names(winner_dvs)) {
      if (all(is.na(sub[[dv]]))) next
      f <- as.formula(paste(dv, "~ econ_index + factor(wave) +", controls))
      m <- tryCatch(lm(f, data = sub), error = function(e) NULL)
      if (is.null(m)) next
      winner_app[[paste(cntry, grp, dv, sep = "_")]] <-
        extract_econ_coef(m) |>
        mutate(dv = dv, dv_label = winner_dvs[dv],
               country = cntry, group = grp, n = nobs(m))
    }
  }
}
winner_loser_subgroup_app <- bind_rows(winner_app)

cat("\nwinner_loser_subgroup_app rows:", nrow(winner_loser_subgroup_app), "\n")
```
...
```{r save}
model_results <- list(
  # --- Appendix-compatible: 5 individual DVs with HC2 SEs ---
  wave_by_wave          = wave_by_wave_app,
  pooled_indiv          = pooled_indiv_app,
  cross_country         = cross_country_app,
  polint_subgroup       = polint_subgroup_app,
  winner_loser_subgroup = winner_loser_subgroup_app,

  # --- Main text figures / inline stats ---
  wave_composite     = wave_df,          # sat/qual/auth indices, wave-by-wave
  pooled_interaction = pooled_df,        # wave × econ interaction models
  cross_country_comp = xc_df,            # cross-country on composite indices
  subgroups          = subgroup_df,

  # --- Appendix robustness ---
  four_bucket        = app_b_df,
  ...
  inline = list(
    ...
    xc_auth_interaction_p     = xc_auth_tidy$p.value[xc_auth_tidy$term == "econ_index:is_korea"],
    xc_auth_interaction_b     = xc_auth_tidy$estimate[xc_auth_tidy$term == "econ_index:is_korea"]
  )
)
