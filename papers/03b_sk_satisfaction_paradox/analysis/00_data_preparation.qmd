...
dat <- dat |>
  ...
  mutate(
    ...
    # --- Authoritarian rule items (raw, for orientation check) ---
    # IMPORTANT: Verify orientation before using auth_reject_* vars.
    # If scale is 1=very bad, 4=very good (for the authoritarian alternative),
    # then higher = MORE authoritarian support → we reverse to get rejection.
    # If scale is 1=very good, 4=very bad, higher already = rejection → no reverse needed.
    # Run the diagnostic below and check against the ABS codebook.
    strongman_raw   = strongman_rule,
    military_raw    = military_rule,
    expert_raw      = expert_rule,
    singleparty_raw = single_party_rule,

    # Normalized without reversal (for nostalgia analysis — higher = more support)
    ...
    # Authoritarian REJECTION (reverse so higher = more rejection of auth rule)
    # Formula: max_val + 1 - x reverses a 1-to-max scale
    # If auth vars are 1=very bad...4=very good, max=4:  4+1-4=1 (very good→1), 4+1-1=4 (very bad→4)
    # That gives higher = more bad evaluation = more rejection. Correct.
    # CHECK: If auth vars already have higher = rejection, set these equal to strongman_n etc.
...
dat <- dat |>
  ...
  mutate(
    ...
    # Authoritarian rejection index (4 items, higher = more rejection)
    auth_reject_index = mean(c_across(c(strongman_reject_n, military_reject_n,
                                         expert_reject_n, singleparty_reject_n)),
                             na.rm = TRUE),

    # Auth nostalgia index (4 items, higher = more support for authoritarianism)
    auth_index = mean(c_across(c(strongman_n, military_n,
                                  expert_n, singleparty_n)),
                      na.rm = TRUE),
  ) |>
  ungroup()
```
...
## Auth variable orientation check

```{r auth-orientation}
cat("=== AUTHORITARIAN VARIABLE ORIENTATION CHECK ===\n")
cat("Check: do higher values = more support for authoritarianism (need reversal)?\n")
cat("Or do higher values = more rejection (no reversal needed)?\n\n")

cat("Korea Wave 1 — strongman_rule value frequencies:\n")
dat |> filter(country_label == "Korea", wave == 1) |>
  count(strongman_raw) |> print()

cat("\nIf 1='very good way to govern' and 4='very bad way' → higher = rejection → NO reversal needed.\n")
cat("If 1='very bad way' and 4='very good way' → higher = support → reversal needed (current code).\n\n")

cat("Auth rejection index summary:\n")
dat |>
  group_by(country_label) |>
  summarise(
    mean_reject = mean(auth_reject_index, na.rm = TRUE),
    mean_support = mean(auth_index, na.rm = TRUE),
    .groups = "drop"
  ) |> print()

cat("\nIf mean_reject + mean_support ≈ 1.0, the variables are properly mirror images.\n")

cat("\n=== Authoritarian rejection coverage ===\n")
dat |>
  group_by(country_label, wave) |>
  summarise(
    n = n(),
    ...
    singleparty_ok  = sum(!is.na(single_party_rule)),
    auth_reject_ok  = sum(!is.na(auth_reject_index)),
    .groups = "drop"
  ) |>
  print(n = 20)
```
