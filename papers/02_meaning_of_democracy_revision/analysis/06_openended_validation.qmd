...
**Related scripts:**
- `01_data_preparation_mlogit.qmd` - Main data preparation
- `02_multinomial_analysis.qmd` - Main multinomial logit analysis
- `05_robustness.qmd` - Other robustness checks
...
here::i_am("papers/02_meaning_of_democracy_revision/analysis/06_openended_validation.qmd")

# Output directory (consistent with other analysis scripts)
output_dir <- here("papers", "meaning-of-democracy-revision", "analysis", "revised", "data")
...
# Save plot
ggsave(
  here("papers", "meaning-of-democracy-revision", "figures", "fig6_validation_openended_vs_q92.png"),
  p, width = 8, height = 6, dpi = 300
)
ggsave(
  file.path(results_dir, "fig_validation_openended_vs_q92.png"),
  p, width = 8, height = 6, dpi = 300
)
```
...
write_csv(country_summary, file.path(results_dir, "w2_openended_country_summary.csv"))

# Save code mappings for reference
code_mappings <- list(
  procedural = procedural_codes,
  substantive = substantive_codes,
  excluded = excluded_codes
)
saveRDS(code_mappings, file.path(output_dir, "openended_code_mappings.rds"))

cat("Saved:\n")
cat("  - w2_openended_coded.rds (", nrow(w2_export), " rows)\n")
cat("  - w2_openended_country_summary.csv (", nrow(country_summary), " countries)\n")
cat("  - openended_code_mappings.rds (code classification lists)\n")
```
