...
here::i_am("papers/02_meaning_of_democracy_revision/analysis/01_data_preparation_mlogit.qmd")
...
# Add electoral fairness variable
# Scale: 1="Not free or fair", 2="...major problems", 3="...minor problems", 4="Completely free and fair"
# Dichotomize: unfair (1-2) vs fair (3-4)
w346_data <- w346_data %>%
  mutate(
    election_unfair = case_when(
      election_free_fair %in% 1:2 ~ 1L,   # not free/fair OR major problems
      election_free_fair %in% 3:4 ~ 0L,   # minor problems OR completely fair
      TRUE ~ NA_integer_
    )
  )

cat("\n=== ELECTORAL FAIRNESS (election_free_fair) ===\n")
cat("Valid responses:", sum(!is.na(w346_data$election_unfair)), "\n")
cat("Missing:", sum(is.na(w346_data$election_unfair)), "\n")
cat("Perceive unfair:", sum(w346_data$election_unfair == 1, na.rm = TRUE),
    sprintf("(%.1f%%)\n", mean(w346_data$election_unfair, na.rm = TRUE) * 100))
...
w346_analysis <- w346_data %>%
  select(
    ...
    age, female, education, urban,
    # Electoral fairness
    election_free_fair, election_unfair,
    # DV items
...
| `wave_year` | 2011, 2015, or 2022 (representative year for each wave) |
| `country_name` | Country name |
| `loser` | 1 = loser, 0 = winner |
| `winner_loser` | "Loser" or "Winner" |
