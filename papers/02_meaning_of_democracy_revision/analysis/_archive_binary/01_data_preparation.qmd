...
here::i_am("papers/02_meaning_of_democracy_revision/analysis/01_data_preparation.qmd")
...
# IMPORTANT: Also filter to only voters (voted_last_election == 1)
# W2 has ~654 cases with electoral_status but voted_last_election != 1
combined <- abs %>%
  filter(wave %in% c(2, 3, 4, 6)) %>%
  filter(voted_last_election == 1) %>%  # Only include actual voters
...
coverage %>%
  ...
  knitr::kable(caption = "Country × Wave Coverage Matrix")
```
