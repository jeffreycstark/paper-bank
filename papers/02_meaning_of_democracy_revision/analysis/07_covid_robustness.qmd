---
title: "COVID Robustness Check — Wave 6"
...
here::i_am("papers/02_meaning_of_democracy_revision/analysis/revised/07_covid_robustness.qmd")
...
# Load master data for COVID variables (from survey-data-prep repo)
source(here("_data_config.R"))
abs_master <- readRDS(abs_harmonized_path)
```
...
# Need to identify matching keys — check what's available
cat("w346 columns:\n")
cat(paste(names(w346), collapse = ", "))
```

```{r check-merge-keys}
# Check available identifiers for merging
# The w346 data likely has country, wave, and some row identifier
w6_analysis <- w346 %>% filter(wave == 6)

cat("\nW6 analysis rows:", nrow(w6_analysis), "\n")
cat("W6 analysis columns:", paste(names(w6_analysis), collapse = ", "), "\n")

# Try to merge on country + wave + row_id if available
# If row_id isn't in w346, we may need to merge on country + wave and match differently
if ("row_id" %in% names(w6_analysis)) {
  ...
  # Fallback: merge COVID vars from master using whatever ID is available
  cat("Available ID columns in w346:\n")
...
estimate_set_ames <- function(data, set_num, controls = "base") {
  ...
  tryCatch({
    ...
    # Compute AMEs for loser variable
    # Using prediction approach
    d0 <- d1 <- model_data
    d0$loser <- 0
    d1$loser <- 1
    ...
    if (is.null(dim(pred0))) {
      # Single column — shouldn't happen with multinomial
      return(NULL)
    }
...
```{r summary-stats}
if (nrow(all_results) > 0) {
  # You'll need to map item numbers to types (procedural/substantive/governance)
  # based on item_meta. For now, print the raw comparison
  cat("\n=== Summary for manuscript paragraph ===\n")
  cat("Fill in the X.X placeholders in Edit 4 with these values:\n\n")

  # Mean AME across procedural items (base vs COVID)
  # NOTE: You'll need to tag items by type. The choice values (1,2,3,4)

  # correspond to specific items in each set per item_meta.
  # This will need to be matched to your item_metadata.rds
...
if (nrow(thai_w6) > 50) {
  ...
  if (nrow(thai_base) > 0 & nrow(thai_covid) > 0) {
    ...
    cat("Report this in the manuscript as: 'Thailand's small Wave 6 sample (n = ",
        nrow(thai_w6), ") precludes separate estimation with COVID controls, ",
        "but the pooled results with Thailand included show...'\n", sep = "")
  }
}
```
...
if (nrow(all_results) > 0) {
  ...
  # Save typed summary for manuscript inline references
  item_types <- item_meta %>%
    filter(grepl("^Set", set)) %>%
    select(set, value, item_type)

  covid_summary <- all_results %>%
    mutate(choice = as.integer(choice)) %>%
    left_join(item_types, by = c("set", "choice" = "value")) %>%
    group_by(item_type, controls) %>%
    summarise(mean_ame = mean(ame), .groups = "drop")

  saveRDS(covid_summary, file.path(results_dir, "robustness_covid_summary.rds"))
