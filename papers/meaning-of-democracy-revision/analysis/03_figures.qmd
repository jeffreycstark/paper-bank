...
here::i_am("papers/meaning-of-democracy-revision/analysis/03_figures.qmd")

# Directories
results_dir <- here("papers", "meaning-of-democracy-revision", "analysis", "revised", "results")
figures_dir <- here("papers", "meaning-of-democracy-revision", "figures")
...
# Load country-wave results (from 04_country_wave.qmd output)
cw_results_file <- file.path(results_dir, "country_wave_results.rds")
if (file.exists(cw_results_file)) {
  cw_results <- readRDS(cw_results_file)
  cw_ames <- cw_results$cw_ames
  cw_gap <- cw_results$cw_gap
  cat("Country-wave results loaded:", nrow(cw_ames), "item-country-wave estimates\n")
} else {
  cw_ames <- NULL
  cw_gap <- NULL
  cat("Country-wave results not found - run 04_country_wave.qmd first for trajectory figures\n")
}
...
# Figure 1b: Main Results - Faceted by Question Set

This version facets by question set (W2, Set1-4) to clarify that these are distinct measurements.
Items labeled "Elections" in W2 and "Free Elections" in Set1 are from different survey questions
administered in different waves.

```{r fig1b-faceted, fig.width=12, fig.height=10}
# Add wave information to distinguish question sets
all_results_faceted <- all_results %>%
  mutate(
    # Create more informative set labels
    set_label = case_when(
      set == "W2" ~ "Wave 2 (2006)\nq92: Most essential",
      set == "Set1" ~ "Waves 3/4/6 (2011-2022)\nSet 1: q85",
      set == "Set2" ~ "Waves 3/4/6 (2011-2022)\nSet 2: q86",
      set == "Set3" ~ "Waves 3/4/6 (2011-2022)\nSet 3: q87",
      set == "Set4" ~ "Waves 3/4/6 (2011-2022)\nSet 4: q88",
      TRUE ~ set
    ),
    set_label = factor(set_label, levels = c(
      "Wave 2 (2006)\nq92: Most essential",
      "Waves 3/4/6 (2011-2022)\nSet 1: q85",
      "Waves 3/4/6 (2011-2022)\nSet 2: q86",
      "Waves 3/4/6 (2011-2022)\nSet 3: q87",
      "Waves 3/4/6 (2011-2022)\nSet 4: q88"
    )),
...
p1b <- ggplot(all_results_faceted,
              aes(x = ame * 100, y = item_label, color = item_type)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  ...
  labs(
    x = "Loser Effect (percentage points)",
    y = NULL,
    title = "Effect of Loser Status on Item Choice Probability by Question Set",
    subtitle = "Positive = losers more likely to choose; Negative = winners more likely\nEach panel shows items from a distinct survey question",
    caption = "Note: 95% CIs from cluster bootstrap. W2 uses different question wording than W3/4/6 sets."
  ) +
...
ggsave(file.path(figures_dir, "fig1b_loser_effects_by_set.png"), width = 12, height = 10, dpi = 300)
ggsave(file.path(figures_dir, "fig1b_loser_effects_by_set.pdf"), width = 12, height = 10)
ggsave(file.path(results_dir, "fig_loser_effects_by_set.png"), width = 12, height = 10, dpi = 300)
ggsave(file.path(results_dir, "fig_loser_effects_by_set.pdf"), width = 12, height = 10)

cat("\nFigure 1b (faceted by set) saved\n")
```
...
# Skip if country-wave results not available
if (is.null(cw_gap)) {
  cat("Skipping trajectory figures - run 04_country_wave.qmd first\n")
} else {
...
} # end else block for cw_gap null check
```
...
if (is.null(cw_gap)) {
  cat("Skipping - run 04_country_wave.qmd first\n")
} else {
...
} # end else block for cw_ames null check
```
...
cat("Files created:\n")
cat("  - fig1_loser_effects_all_items.{png,pdf}    (all items, single panel)\n")
cat("  - fig1b_loser_effects_by_set.{png,pdf}      (faceted by question set) ★ NEW\n")
...
cat("\nNote: Figure 1b clarifies that W2 items and W3/4/6 set items are from\n")
cat("      distinct survey questions with different wording.\n")
```
