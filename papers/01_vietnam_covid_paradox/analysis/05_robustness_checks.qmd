...
# Check if individual trust items exist (require ≥50% coverage in Vietnam)
vietnam_filtered <- data %>% filter(country_name == "Vietnam")
trust_candidates <- grep("^trust|^covid.*trust", names(data), value = TRUE)
trust_items <- trust_candidates[sapply(trust_candidates, function(v) {
  mean(!is.na(vietnam_filtered[[v]])) >= 0.5
})]
...
# Harmonized data is already reverse-coded (4 = most trust); no recode needed.
# general_trust: president, national gov, parliament, civil service, police
# covid_trust: covid_trust_info (reverse-coded in 00_data_preparation.R)

df_resid <- data %>%
  filter(country_name %in% c("Cambodia", "Thailand", "Vietnam")) %>%
  rowwise() %>%
  mutate(
    general_trust = mean(c(trust_president, trust_national_government,
                           trust_parliament, trust_civil_service,
                           trust_police), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  filter(!is.na(general_trust) & !is.na(covid_trust_info))

# Check sample sizes
cat("\n=== Sample Sizes for Residual Analysis ===\n")
df_resid %>% count(country_name) %>% print()

# Predict COVID trust from general institutional trust
model_residual <- lm(covid_trust_info ~ general_trust, data = df_resid)

# Extract residuals
df_resid <- df_resid %>%
  mutate(
    predicted = predict(model_residual, newdata = .),
    residual  = covid_trust_info - predicted
  )
...
residual_summary <- df_resid %>%
  ...
  summarise(
    ...
    mean_covid_trust = mean(covid_trust_info, na.rm = TRUE),
...
ggplot(df_resid, aes(x = general_trust, y = covid_trust_info, color = country_name)) +
...
saveRDS(list(
  ...
  residuals_df = df_resid %>% select(country_name, general_trust, covid_trust_info, predicted, residual)
