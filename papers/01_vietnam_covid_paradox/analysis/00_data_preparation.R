...
d <- d |>
  mutate(
    ...
    income_adequacy   = as.numeric(income_fairness),          # subjective income adequacy
    educ_level        = as.numeric(education_level),          # 1-10 ordinal
    urban             = as.integer(urban_rural),              # 0=rural, 1=urban
    year              = as.integer(int_year),                 # interview year
    month             = as.integer(int_month)                 # interview month
  )
...
output_path <- here::here(
  "papers", "01_vietnam_covid_paradox", "analysis", "data", "analysis_data.rds"
)
