...
```{r model-factor}
m_factor <- lm(corruption ~ factor(wave_num) * type, data = abs_long)
summary(m_factor)
```
...
contrasts_out <- hypotheses(
  m_factor,
  hypothesis = paste0(
    "(factor(wave_num)5:typeexperiential - factor(wave_num)4:typeexperiential) - ",
    "(factor(wave_num)5 - factor(wave_num)4) = 0"
  )
)
