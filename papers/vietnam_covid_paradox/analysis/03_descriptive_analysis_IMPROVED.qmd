...
here::i_am("papers/vietnam_covid_paradox/analysis/03_descriptive_analysis_IMPROVED.qmd")
...
data_path <- here("papers", "vietnam_covid_paradox", "analysis", "data", "analysis_data.rds")
...
write_csv(completeness_vars, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "variable_completeness.csv"))
...
write_csv(completeness_country, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "completeness_by_country.csv"))
...
gtsave(complete_case_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "complete_case_summary.html"))
...
gtsave(paradox_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table1_vietnam_paradox.html"))
...
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig1_vietnam_paradox.png"), 
...
gtsave(descriptive_stats, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table1_descriptive_stats.html"))
...
p4 <- ggplot(ab_analysis, aes(x = emergency_powers_support, y = country_name, 
                               fill = country_name)) +
  geom_density_ridges(alpha = 0.7, scale = 0.9) +
  scale_fill_manual(values = country_colors) +
  labs(title = "Emergency Powers Support", x = "Support (1-4)", y = NULL) +
  theme_ridges() +
  theme(legend.position = "none")
...
print(combined_dist)
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig2_distributions.png"), combined_dist, 
...
# Save table
gtsave(reliability_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table2_reliability.html"))
...
for (var in names(vars_to_plot)) {
  ...
  ggsave(
    here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", paste0("fig_dist_", var, ".png")),
...
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig3_covid_approval.png"), width = 10, height = 6, dpi = 300)
...
covid_approval_data %>%
  ...
  gtsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table2_covid_approval_cor.html"))
...
gtsave(economic_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table_covid_impacts.html"))
...
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig_economic_approval.png"), width = 10, height = 6, dpi = 300)
...
gtsave(economic_cor_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table_economic_approval_cor.html"))
...
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig_impact_comparison.png"), width = 12, height = 5, dpi = 300)
...
gtsave(comparison_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table_impact_comparison.html"))
...
ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig4_trust_approval.png"), width = 10, height = 6, dpi = 300)
...
trust_approval_data %>%
  ...
  fmt_number(columns = correlation, decimals = 3) %>%
  gtsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table3_trust_approval_cor.html"))
...
# Save table
gtsave(effect_size_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table5_effect_sizes.html"))
...
# Save table
gtsave(bootstrap_table, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table6_bootstrap_correlations.html"))
...
cor_vars <- ab_analysis %>%
  ...
  rename(
    ...
    "Emergency Powers" = emergency_powers_support
  )
...
# Visualize
png(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig5_correlation_matrix.png"), width = 800, height = 800)
...
gtsave(correlation_matrix, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "table4_correlation_matrix.html"))
...
# Compare support across different emergency scenarios
emergency_comparison <- ab_analysis %>%
  group_by(country_name) %>%
  summarise(
    `COVID Crisis` = mean(emergency_q172a >= 3, na.rm = TRUE) * 100,
    `Economic Crisis` = mean(emergency_q172b >= 3, na.rm = TRUE) * 100,
    `Corruption` = mean(emergency_q172c >= 3, na.rm = TRUE) * 100,
    `Security Crisis` = mean(emergency_q172d >= 3, na.rm = TRUE) * 100,
    `War` = mean(emergency_q172e >= 3, na.rm = TRUE) * 100
  ) %>%
  pivot_longer(-country_name, names_to = "scenario", values_to = "support")

ggplot(emergency_comparison, aes(x = scenario, y = support, fill = country_name)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  scale_fill_manual(values = country_colors, name = "Country") +
  scale_y_continuous(limits = c(0, 100), labels = scales::percent_format(scale = 1)) +
  labs(title = "Support for Emergency Government Powers by Scenario",
       subtitle = "General emergency powers (all three countries)",
       x = "Emergency Scenario", y = "% Approve") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(here("papers", "vietnam_covid_paradox", "analysis", "output", "figures", "fig6_emergency_powers.png"), width = 12, height = 6, dpi = 300)
cat("✓ Saved: figures/fig6_emergency_powers.png\n")
```
...
write_csv(summary_stats, here("papers", "vietnam_covid_paradox", "analysis", "output", "tables", "summary_statistics.csv"))
