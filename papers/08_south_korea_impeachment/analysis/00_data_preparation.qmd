...
korea_abs <- korea_abs |>
  mutate(
    ...
    # ABS W3-W6: "To what extent is Korea governed democratically?" (1-4)
    dem_quality = if_else(!is.na(democracy_quality), democracy_quality,
                  if_else(!is.na(democracy_extent), democracy_extent, NA_real_)),

    # --- Government responsiveness ---
    # Whether the government is responsive to what citizens want (1-4)
    govt_resp = if_else(!is.na(govt_responsiveness), govt_responsiveness,
                if_else(!is.na(govt_responsive), govt_responsive, NA_real_)),

    # --- Economic evaluation (current conditions) ---
    # 1=Very bad, 4=Very good  (or 1-5 scale normalized below)
    econ_eval_raw = if_else(!is.na(econ_present), econ_present,
                    if_else(!is.na(econ_situation), econ_situation, NA_real_)),

    # --- Institutional trust (1-4 scale, higher = more trust) ---
    # Executive
    trust_exec = trust_central_govt,

    # Horizontal accountability (legislative/judicial)
    trust_legis = if_else(!is.na(trust_national_assembly), trust_national_assembly,
                  if_else(!is.na(trust_legislature), trust_legislature, NA_real_)),
    trust_courts_abs = if_else(!is.na(trust_courts), trust_courts,
                       if_else(!is.na(trust_judiciary), trust_judiciary, NA_real_)),

    # Societal
    trust_media_abs = trust_media,
...
kamos_raw <- kamos_raw |>
  mutate(
    # --- Economic evaluation (1-5, higher = better) ---
    econ_present_raw = if_else(!is.na(econ_present), econ_present,
                       if_else(!is.na(economy_rating), economy_rating,
                       if_else(!is.na(econ_eval), econ_eval, NA_real_))),
    econ_present_n = normalize_01(econ_present_raw),

    # Economic composite (present + national economy if both available)
    econ_composite_n = if_else(
      !is.na(econ_national) & !is.na(econ_present_raw),
      rowMeans(cbind(normalize_01(econ_present_raw),
                     normalize_01(econ_national)), na.rm = TRUE),
      normalize_01(econ_present_raw)
    ),
...
kamos_long <- kamos_wide |>
  ...
  mutate(
    ...
    # Keep raw trust score for ordered logit robustness check
    trust_score_raw = recode(institution,
      trust_cg_n = trust_cg_raw,
      trust_lg_n = trust_lg_raw,
      trust_na_n = trust_na_raw,
      trust_md_n = trust_md_raw,
      trust_ng_n = trust_ng_raw
    )
  )
