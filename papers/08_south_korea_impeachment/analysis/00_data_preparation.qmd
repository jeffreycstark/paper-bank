...
kamos_raw <- kamos_raw |>
  mutate(
    ...
    # Category D: excluded from primary analysis (placebo test and direction table)
    trust_pe_raw = trust_private_enterprise,
    trust_ri_raw = trust_religious,

    # Robustness check items: generalized social trust
    trust_sc_raw = trust_society,
    trust_ct_raw = trust_citizens,
    ...
    trust_pe_n = normalize_01(trust_pe_raw),
    trust_ri_n = normalize_01(trust_ri_raw),
    trust_sc_n = normalize_01(trust_sc_raw),
    trust_ct_n = normalize_01(trust_ct_raw),
...
kamos_wide <- kamos_raw |>
  select(
    ...
    # Category D (excluded from primary; used in placebo test)
    trust_pe_raw, trust_ri_raw, trust_pe_n, trust_ri_n,
    # Social trust robustness items
    trust_sc_raw, trust_ct_raw, trust_sc_n, trust_ct_n
  )
