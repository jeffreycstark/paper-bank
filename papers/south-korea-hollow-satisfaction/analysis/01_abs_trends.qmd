...
qual_means <- kor |>
  ...
  mutate(
    ...
    label   = recode(variable,
      ...
      qual_current_govt     = "Current government's democratic rating",
...
econ_means <- kor |>
  ...
  pivot_longer(starts_with("econ_") | starts_with("econ_"),
               names_to = "variable", values_to = "value") |>
  # pick only our four economic vars (avoid re-picking sat_ or qual_ vars)
  filter(variable %in% c("econ_national", "econ_hh_sat", "econ_outlook", "econ_status")) |>
  filter(!is.na(value)) |>
  group_by(wave, survey_year, variable) |>
  summarise(
    mean = weighted.mean(value, weight, na.rm = TRUE),
  ...
  mutate(
    ...
    label   = recode(variable,
      econ_national = "Current national economic conditions",
      econ_hh_sat   = "Household income satisfaction",
      econ_outlook  = "Economic outlook (next year)",
      econ_status   = "Subjective social status"
    )
  )
