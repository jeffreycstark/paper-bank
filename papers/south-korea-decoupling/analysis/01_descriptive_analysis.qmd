...
calc_slope <- function(data, yvar, year_var = "year") {
  d <- data |> select(y = all_of(yvar), x = all_of(year_var)) |> drop_na()
  if (nrow(d) < 3) return(tibble(slope = NA_real_, slope_decade = NA_real_, n_waves = nrow(d)))
  m <- lm(y ~ x, data = d)
  tibble(
    slope        = coef(m)["x"],
    slope_decade = coef(m)["x"] * 10,
    n_waves      = nrow(d)
  )
}
