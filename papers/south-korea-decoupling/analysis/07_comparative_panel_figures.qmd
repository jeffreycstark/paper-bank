...
```{r wave-means}
compute_wave_means <- function(data, country, years) {
  data |>
...
```{r taiwan-slopes}
calc_slope <- function(wm, var) {
  d <- wm[!is.na(wm[[var]]) & !is.na(wm$year), ]
  if (nrow(d) < 3) {
    return(tibble(term = "year", estimate = NA_real_, std.error = NA_real_,
                  p.value = NA_real_, per_decade = NA_real_))
  }
  m <- lm(as.formula(paste(var, "~ year")), data = d)
  tidy(m) |>
    filter(term == "year") |>
    mutate(per_decade = estimate * 10)
}
