...
for (outcome in primary_outcomes) {
  ...
  m <- lm(f_adj, data = hk5_analysis, weights = weight)
  print(summary(m))
}
```
...
# Compute weighted Cohen's d for each
...
# --- Part B: Effect sizes for ordinal/continuous items (weighted) ---
dem_effects <- map_dfr(dem_items$variable, function(var) {
  ...
  if (item_type %in% c("ordinal", "binary")) {
    ...
    tibble(
      ...
      cohens_d = d,
      p_value = coef_row["Pr(>|t|)"],
      test_type = "weighted OLS",
      n_protest = nrow(dat_pro),
      n_postnsl = nrow(dat_post)
    )
  } else {
    # Nominal: chi-squared test (unweighted — counts are counts)
    ct <- table(
      c(rep("Protest", nrow(dat_pro)), rep("Post-NSL", nrow(dat_post))),
      c(dat_pro[[var]], dat_post[[var]])
    )
    chi <- chisq.test(ct)
