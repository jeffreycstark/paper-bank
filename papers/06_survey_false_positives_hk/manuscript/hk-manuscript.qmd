...
abstract: |
  Scholars of democratic backsliding rely on survey data to track regime legitimacy, yet
  the same repressive forces that dismantle institutions also distort the instruments used
  to measure public attitudes. This article develops and validates a "sensitivity gradient"
  framework—a portable diagnostic for identifying measurement distortion in standard survey
  batteries without experimental design. The framework exploits a theoretical prediction:
  because the cost of honest response varies predictably across survey items as a function
  of institutional coerciveness, preference falsification generates a systematic within-instrument
  gradient. High-sensitivity items (e.g., trust in police) inflate due to strategic compliance
  and the selective exit of critics; low-sensitivity items (e.g., abstract democratic values)
  remain comparatively robust. The divergence itself constitutes a false positive for regime
  support. Testing this framework using a natural quasi-experiment in Hong Kong (the 2020
  National Security Law), the data reveal a "trust paradox" consistent with measurement
  distortion: trust in police surged ($d \approx +0.49$) while perceived democratic suitability
  collapsed ($d = -0.54$), and the canonical "critical citizens" correlation reverses—a second
  diagnostic signal that standard survey relationships have broken down. Cross-national
  replication using World Values Survey data from Turkey ($r = -0.68$), Latinobarómetro
  data from Venezuela and Nicaragua, and Afrobarometer data from Burkina Faso establishes
  both external validity and discriminant validity: the gradient appears when repression is
  targeted and procedurally legitimated, and is absent or inverted when institutional collapse
  is genuine. The resulting diagnostic toolkit—five portable signals computable from standard
  ABS, WVS, Afrobarometer, and Latinobarómetro batteries—offers researchers a practical
  method for assessing whether survey-based legitimacy claims in autocratizing contexts can
  be trusted to mean what they appear to say.
keywords:
  - survey measurement
  - autocratization
...
I adjudicate among competing mechanisms through age-stratified analysis, information environment tests, and rally effect diagnostics (Section 4.5), finding that no single alternative explanation accounts for all observed patterns. I then apply the same framework to Turkey using World Values Survey data spanning the 2016 coup attempt, finding consistent evidence of the gradient across a different autocratization context and survey instrument (Section 4.6). The framework is best suited to contexts where autocratization is rapid and survey fieldwork spans the transition, though cross-wave comparisons (as in Turkey) also reveal gradient patterns when waves bracket clear autocratization shocks. The approach requires only standard trust and democratic evaluation items available in all major cross-national survey programs.

These findings contribute to three literatures. First, and most directly, they offer
a practical methodological advance for the study of democratic backsliding
[@Waldner2018-yw]: a diagnostic toolkit that can be applied retrospectively to data
already collected in ABS, WVS, Afrobarometer, and Latinobarómetro archives, without
experimental design, and that has now been validated in five distinct political
contexts.^[Preliminary evidence from published Levada Center tracking polls in Russia
shows patterns consistent with the framework (trust increases coinciding with democratic
support decreases around the February 2022 invasion of Ukraine), but individual-level
microdata access would be required for the item-level diagnostics that constitute the
key tests. Online Appendix J reports available WVS-based evidence for Russia, where
Waves 6 (2011) and 7 (2017) bracket the post-Crimea annexation period rather than the
2022 invasion.] Second, the findings engage debates on authoritarian legitimacy
[@Nathan2003-wg; @Tang2016-nl; @Dukalskis2017-md], providing empirical grounds to
question, in at least some autocratizing contexts, whether measured "performance legitimacy"
reflects genuine public approval or is partly an artifact of survey non-response and
preference falsification. Third, they bridge
critical theory and survey methodology, operationalizing the concern that quantitative
metrics can inadvertently reproduce the logic of domination by masking the silence of
the marginalized—and offering a concrete means of detection.
...
The micro-level logic connecting repression to differential survey response runs through three sequential steps. First, autocratization shifts the perceived cost of honest response asymmetrically across item types. When a regime deploys coercive institutions visibly against political opponents, respondents update their beliefs about the risk of expressing dissatisfaction with those specific institutions: the police who arrested protesters, the government that enacted emergency legislation. Abstract evaluations of "democracy as a system" carry lower perceived risk because they are harder for authorities to interpret as directed criticism and because regimes typically claim democratic legitimacy themselves, making abstract democratic endorsements compatible with compliance. Second, this cost asymmetry produces two behavioral responses. Among respondents who remain in the survey pool, high-stakes items elicit strategic misrepresentation: respondents report regime-favorable values not because their attitudes have changed but because honest expression now carries costs they are unwilling to bear [@Kuran1995-up; @Simpser2013-pf]. Among the subset of respondents most likely to express dissent---those with the strongest prior dissatisfaction---the elevated cost of participation induces selective exit: refusal, emigration, or withdrawal from public life [@Kobayashi2022-ih]. Both responses inflate measured support on high-sensitivity items. Third, the two responses generate *different* observable signatures that allow partial identification. Pure falsification predicts that mean responses shift while variance decreases (respondents cluster at the regime-favorable end); pure selection predicts that mean responses shift while variance is preserved or increases (the distribution truncates from the low end). The sensitivity gradient itself---the rank-order correlation between item coerciveness and observed shift---is predicted by both mechanisms jointly, since neither predicts uniform shifts across all item types. Distinguishing their relative contributions requires the additional tests reported in Sections 4.4 and 4.5, but the gradient pattern is a necessary implication of either mechanism operating in isolation or in combination.

## Item-Level Asymmetry Within Sensitivity Categories {#sec-item-asymmetry}

The sensitivity gradient framework assigns items to broad categories based on the coercive salience of the institution being evaluated. Within categories, however, item wording introduces a second source of variation that the framework must accommodate. Two items nominally measuring the same construct — democratic commitment — can face asymmetric falsification pressure depending on whether the regime-favorable response is cognitively available to the respondent.

Consider the contrast between "democracy is always preferable to any other form of government" and "democracy suits a country like ours." The first item invites an abstract affirmation. Under authoritarian consolidation, this affirmation is compatible with regime loyalty: the Chinese Communist Party and the post-NSL Hong Kong government both claim the mantle of democracy, however defined, making "democracy is always preferable" a statement that carries minimal political risk. The second item asks respondents to evaluate whether democracy *fits their specific political context* — an evaluation that, in Hong Kong's post-NSL environment, is functionally a referendum on Beijing's governance. The regime-favorable response to this item is not affirmation but denial, and denial requires the respondent to actively endorse the dismantling of the institutions they previously inhabited. This is psychologically and politically costlier than simply declining to endorse an abstraction.

This distinction maps onto what @Wedeen2000-bg calls performative compliance: regimes require citizens to act *as if* they support the system, and the specific performance demanded varies by item. Affirming "democracy is always preferable" is a performance compatible with the regime's own democratic self-description; it signals compliance without requiring genuine belief revision. Affirming that "democracy does not suit Hong Kong" requires endorsing a specific claim about Hong Kong's political capacity — one that directly contradicts the experience of respondents who lived through the pre-NSL democratic period. The cognitive cost of this endorsement is higher, making falsification less complete. The prediction, then, is not just that low-sensitivity items show smaller falsification-consistent shifts than high-sensitivity items, but that within the low-sensitivity category, items requiring performance-compatible affirmations will shift differently from items requiring performance-incompatible ones.

This item asymmetry argument generates a testable micro-level prediction: *dem_always_preferable* should shift in the regime-favorable direction (upward) while *democracy_suitability* should shift in the regime-unfavorable direction (downward), despite both being nominally classified as low-sensitivity democratic commitment items. The robustness analysis in Online Appendix D.3 confirms this pattern: *dem_always_preferable* increases post-NSL ($d = +0.36$) while *democracy_suitability* collapses ($d = -0.52$). Far from undermining the sensitivity gradient, this within-category divergence refines it — falsification operates not just at the level of institutional coerciveness but at the level of whether the item's wording makes strategic compliance cognitively accessible.

## The "Critical Citizens" Paradigm and Its Assumptions

The "critical citizens" framework [@Norris2011-oc; @Easton1975-bg; @Dalton1984-bt] holds that citizens in consolidated democracies combine commitment to democratic ideals with dissatisfaction toward democratic performance, producing a characteristic negative correlation between democratic support and democratic satisfaction. This relationship is well-documented across OECD countries and developing democracies [@Offe2008-xz; @Claassen2020-hk], and its ubiquity makes its *absence or reversal* theoretically informative: when the standard negative correlation weakens or flips positive, something has changed in the conditions under which respondents express these attitudes. This article proposes that correlation reversal is a diagnostic signal for measurement distortion---an indicator that the cost structure of honest survey response has shifted asymmetrically across item types.
...
This cross-national positioning justifies the single-case focus: Hong Kong represents the extreme case of the measurement problem this article addresses, where the stakes of misinterpretation---treating falsified survey responses as evidence of genuine regime legitimacy---are highest.
...
The information environment account generates a testable prediction, however, that cuts against what the data show. If narrowed media---rather than strategic compliance---drives the trust surge, the gradient should be *flatter* among respondents who maintain high political interest, since these individuals are more likely to seek out alternative information sources and should therefore be less susceptible to belief revision through media restructuring alone. I split the sample by political interest (1--4 scale; "high" defined as 3--4, $N = 315$; "low" as 1--2, $N = 815$) and compute the sensitivity gradient separately for each group. The pattern is the opposite of what the information environment account predicts. Politically interested respondents show a *steeper* gradient: the gap between mean Cohen's $d$ for high-sensitivity items and low-sensitivity items is 0.73 for the high-interest group versus 0.43 for the low-interest group. On the most coercive items, the divergence is stark---trust in police surged by $d = 0.77$ among the politically interested but only $d = 0.32$ among the disengaged; trust in the president/chief executive shows an even wider gap ($d = 0.77$ vs. $d = 0.24$). At the low-sensitivity end, democracy suitability declined more sharply among the interested ($d = -0.68$ vs. $d = -0.43$). A three-way interaction model (post-NSL $\times$ item sensitivity $\times$ political interest) confirms the differential: $\beta = 0.37$, $p = .001$, 95% CI $[0.15, 0.60]$. If media restructuring were the primary driver, the most politically attentive respondents---those with the strongest motivation and capacity to access non-official information---should show the weakest distortion signature. That they show the strongest is difficult to reconcile with belief revision and is instead consistent with strategic compliance: respondents who know the political stakes are precisely those with the most reason to misrepresent on high-sensitivity items. One item-level anomaly reinforces this interpretation. On the "free to speak without fear" item, the high-interest group shows a *smaller* positive shift ($d = 0.19$) than the low-interest group ($d = 0.46$)---an experiential anchoring effect whereby politically engaged respondents, who confront speech restrictions directly, find it harder to falsify on an item that contradicts their lived reality.
...
A more targeted version of the confound---whereby COVID specifically elevated trust in security and enforcement institutions rather than producing uniform approval---is also inconsistent with the observed pattern. If pandemic conditions selectively boosted police and government trust through a security-salience mechanism, we should observe comparable increases in trust in the military, which plays an analogous public-order role. Instead, military trust increased substantially less than police trust ($d = +0.19$ vs. $d = +0.43$), and trust in courts---another institution with direct public-contact functions during the pandemic---was essentially unchanged ($d = +0.05$). The within-trust gradient, ordered precisely by each institution's coercive role in NSL enforcement rather than by COVID-relevant functions, is more consistent with NSL-specific falsification than with a pandemic security-salience effect. Moreover, the item measuring freedom of speech *increased* post-NSL ($d = +0.41$)---an implausible rally-driven improvement in a context where the NSL had criminalized broad categories of political expression. Cross-national comparison provides additional leverage: ABS Wave 5 polities unaffected by comparable political crackdowns show no systematic trust increases during the same period, suggesting that the Hong Kong pattern is not a regional COVID effect. The divergent movement of trust and evaluative items, combined with the within-trust ordering, is the signature of differential item sensitivity, not pandemic-driven approval.

*Differential baseline distrust.* A compositional selection account predicts that items with greater baseline variance in trust---reflecting deeper pre-existing disagreement among respondents---should show the largest post-NSL shifts, as the critics most likely to exit the sample were clustered at the low end of these items. I test this directly by examining whether pre-period item variance independently predicts the magnitude of post-NSL shifts (Cohen's *d*) after accounting for sensitivity rank. It does not: sensitivity rank explains 79% of cross-item variance in shifts ($\rho = -0.946$, $p = .001$, $n = 6$ items), while baseline SD adds no independent predictive power (partial $r = -0.062$, $p = .14$). The clearest discriminating observation involves police and courts, which have nearly identical pre-period standard deviations (police SD = 1.16; courts SD = 1.02) yet diverge dramatically in post-NSL trajectory ($d = +0.46$ vs. $d = +0.05$). Items with similar baseline disagreement shift in proportion to institutional coerciveness, not pre-existing variance---the prediction of falsification, not selection. Full results, including a robustness analysis disaggregating the democratic attitudes index into component items, are reported in Online Appendix D.3.

*Distributional decomposition.* A deeper cut at the falsification-versus-selection question comes from examining the *shape* of the response distribution, not just its mean. The two mechanisms leave distinct distributional fingerprints. Pure falsification predicts ceiling clustering: respondents who would have reported low trust instead report the maximum, producing an increased top-response proportion, a negative skew shift, and---if the clustering is sufficiently concentrated---reduced variance. Pure selection predicts bottom truncation: critics exit the sample, the floor proportion shrinks, but the ceiling proportion need not change because no one is being *added* to the top of the distribution. A joint pattern, in which both the ceiling rises and the floor drops, indicates both mechanisms operating simultaneously. Trust in police exhibits exactly this joint signature. The proportion of respondents selecting the maximum trust response nearly doubled (15.8% $\rightarrow$ 29.0%, $+$13.2 percentage points), while the proportion at the minimum dropped by a comparable magnitude (41.4% $\rightarrow$ 25.9%, $-$15.5pp). Skewness reversed from positive to negative ($+$0.37 $\rightarrow$ $-$0.28), consistent with a mass of responses shifting toward the ceiling. Trust in national government shows the same dual pattern: ceiling $+$11.4pp, floor $-$16.2pp, skewness reversal from $+$0.39 to $-$0.25. Variance itself barely changed for either item (Brown-Forsythe $p = .41$ and $p = .49$, respectively)---the ceiling clustering and floor truncation roughly offset, preserving overall dispersion while fundamentally reshaping the distribution.

Trust in courts provides the discriminating contrast. The ceiling proportion was flat ($+$0.1pp), the floor declined only modestly ($-$2.6pp), skewness was essentially unchanged ($-$0.31 $\rightarrow$ $-$0.30), and variance contracted slightly. This is a distribution that barely moved---exactly what the sensitivity gradient predicts for an institution with low coercive salience. If selection alone were driving the trust surge, it would need to operate with an implausible degree of institutional specificity: critics of police and government would need to exit the sample at five to six times the rate of critics of courts, despite all three items appearing in the same battery. The more parsimonious account is that a baseline selection effect---visible in the modest floor declines across all trust items---is overlaid by institution-specific falsification, concentrated on the items most directly linked to NSL enforcement. The ceiling-floor decomposition tracks the sensitivity ranking almost perfectly: the Spearman correlation between item-level Cohen's $d$ and ceiling change is $\rho = 1.0$ ($p = .017$), and between $d$ and skewness change, $\rho = -1.0$ ($p = .017$).

Taken together, these tests suggest that no single mechanism fully accounts for the observed pattern. Conservative revaluation likely contributes, particularly among older respondents, but the age gradient contradicts it as the dominant driver. Information environment restructuring is a plausible concurrent mechanism---post-NSL respondents do rate China as more democratic, and the China-trust correlation strengthens after media closure---but the political interest stratification cuts against it as the primary driver: the gradient is steepest precisely among respondents best positioned to access alternative information, the opposite of what belief revision through narrowed media would predict. (The strength of the China-trust association, $r = 0.69$, may itself partly reflect falsification; respondents who inflate trust in police may also strategically inflate their assessment of China's democratic credentials.) A COVID rally effect receives little support from the divergent pattern. The evidence is most consistent with a combination of preference falsification and compositional selection, though the present design cannot precisely decompose the relative contributions of these mechanisms or rule out information effects and conservative revaluation as secondary contributors.
...
@tbl-turkey-gradient and @fig-gradient-hk-turkey present the results. The pattern closely mirrors Hong Kong.

That the gradient emerges in Turkey despite the different institutional pathway of autocratization---post-coup emergency decrees and constitutional consolidation rather than externally imposed security legislation---is theoretically informative. The mechanism does not require a specific form of repression; it requires only that the regime deploy coercive institutions visibly against identifiable opponents, creating asymmetric perceived costs of honest response across item types. In Turkey, the mass purges of civil servants, military officers, academics, and journalists following July 2016 established clearly that association with opposition was costly, making trust in the police and armed forces---the institutions most associated with the post-coup crackdown---the highest-stakes items for honest expression. That abstract democratic evaluations declined simultaneously (importance of democracy $d = -0.36$) suggests respondents retained the capacity for honest dissent on less targeted dimensions, consistent with the item-asymmetry logic developed in @sec-item-asymmetry. The attenuation of the gradient relative to Hong Kong ($r = -0.68$ vs. $r = -0.85$) is also interpretable: cross-wave comparisons spanning seven years introduce genuine attitude change alongside measurement distortion, whereas Hong Kong's within-wave design isolates a much shorter window in which genuine preference change is less plausible. The gradient is expected to be noisier in cross-wave designs, and the Turkey result is consistent with this expectation.
...
One potential concern is that the attenuated Turkey gradient reflects instrument-level differences rather than political context. Appendix K cross-instrument validation---comparing ABS trust means to WVS confidence means for Thailand and the Philippines across near-simultaneous fieldwork---shows that institutional rank orderings are broadly preserved across instruments (Thailand Spearman $\rho = 0.60$; Philippines $\rho = 0.10$), suggesting that the modest attenuation ($r = -0.68$ vs. $r = -0.85$) is unlikely to be an artifact of item wording.
...
The sensitivity gradient approach does not resolve every measurement challenge in
authoritarian survey research. It works best when repression is uneven, political change
is rapid, and surveys contain items spanning a range of political sensitivity. The
approach identifies patterns consistent with falsification but cannot precisely decompose
the relative contributions of falsification, selection, and genuine attitude change.
What it does offer is something previously unavailable: a method for applying
falsification-detection logic retrospectively, to the vast archives of standard survey
data already collected under conditions of repression. The sensitivity gradient can be
computed from trust and democratic evaluation items available in every major cross-national
survey program. The critical citizens diagnostic requires only two items. The toolkit
in @tbl-toolkit provides explicit thresholds that researchers can apply without
specialized data collection. For the growing number of cases where the underlying
conditions obtain---not only Hong Kong and Turkey, but potentially Myanmar, Russia, and
other rapidly autocratizing contexts---the framework offers a practical answer to a
consequential question: when survey data from closing spaces report rising regime support,
