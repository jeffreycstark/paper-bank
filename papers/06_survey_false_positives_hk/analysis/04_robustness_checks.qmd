...
# 7. Convergent Validity: Low-Missingness Items and Suitability Missingness Predictors

Democracy suitability exhibits ~60% item-level missingness in both periods, raising
the concern that this item is too unreliable to anchor cross-national comparisons.
This section addresses the concern through two diagnostics: (a) convergent validity
(do low-missingness democracy items move in the same direction as suitability?) and
(b) a cross-tabulation showing that respondents missing on suitability display the
same attitudinal patterns on low-missingness items as those who answered.

## 7.1 Convergent Validity: Direction of Low-Missingness Items

```{r convergent-validity}
# Compare direction and significance of low-missing democracy items
# against high-missing democracy_suitability

convergent_vars <- c(
  "dem_vs_econ",                   # 1.4% missing
  "dem_vs_equality",               # 3.0% missing
  "dem_procedural_vs_substantive", # 4.4% missing
  "dem_free_speech",               # 8.0% missing
  "dem_always_preferable",         # 9.1% missing
  "democracy_satisfaction",        # 11.1% missing
  "democracy_suitability"          # ~60% missing (anchor)
)
convergent_vars <- convergent_vars[convergent_vars %in% names(hk5_analysis)]

convergent_results <- map_dfr(convergent_vars, function(var) {
  protest <- hk5_analysis |> filter(period == "Protest") |> pull(!!sym(var))
  postnsl <- hk5_analysis |> filter(period == "Post-NSL") |> pull(!!sym(var))

  n_total <- length(protest) + length(postnsl)
  n_miss  <- sum(is.na(protest)) + sum(is.na(postnsl))
  pct_miss <- round(100 * n_miss / n_total, 1)

  protest_clean <- na.omit(protest)
  postnsl_clean <- na.omit(postnsl)

  if (length(protest_clean) < 10 | length(postnsl_clean) < 10) return(NULL)

  tt <- t.test(protest_clean, postnsl_clean)
  pooled_sd <- sqrt(((length(protest_clean) - 1) * sd(protest_clean)^2 +
                       (length(postnsl_clean) - 1) * sd(postnsl_clean)^2) /
                      (length(protest_clean) + length(postnsl_clean) - 2))
  d <- (mean(postnsl_clean) - mean(protest_clean)) / pooled_sd

  tibble(
    variable = var,
    pct_missing = pct_miss,
    protest_mean = mean(protest_clean),
    postnsl_mean = mean(postnsl_clean),
    cohens_d = d,
    p_value = tt$p.value,
    direction = ifelse(d > 0, "Post-NSL higher", "Protest higher"),
    dem_priority_declines = (var %in% c("dem_vs_econ", "dem_vs_equality") & d < 0) |
                            (var == "democracy_suitability" & d < 0) |
                            (var == "dem_always_preferable" & d > 0)  # higher = less preferable coding?
  )
})

convergent_results |>
  mutate(across(c(protest_mean, postnsl_mean, cohens_d), ~round(.x, 3)),
         p_value = format.pval(p_value, digits = 3)) |>
  kable(
    col.names = c("Variable", "% Missing", "Protest Mean", "Post-NSL Mean",
                   "Cohen's d", "p-value", "Direction", "Dem Priority ↓?"),
    caption = paste0(
      "Convergent validity: democracy items ordered by missingness rate. ",
      "Low-missingness items (dem_vs_econ, dem_vs_equality) move in the same direction ",
      "as the high-missingness anchor (democracy_suitability), supporting convergent validity."
    ),
    booktabs = TRUE
  ) |>
  kable_styling(latex_options = c("scale_down", "hold_position"))

# Summary: do low-miss items agree with suitability direction?
suit_d <- convergent_results |> filter(variable == "democracy_suitability") |> pull(cohens_d)
low_miss_agree <- convergent_results |>
  filter(variable %in% c("dem_vs_econ", "dem_vs_equality")) |>
  mutate(same_direction = sign(cohens_d) == sign(suit_d))

cat("\nDemocracy suitability Cohen's d:", round(suit_d, 3), "\n")
cat("Low-miss items agree in direction:",
    sum(low_miss_agree$same_direction), "of", nrow(low_miss_agree), "\n")
```

## 7.2 Cross-Tabulation: Suitability Missingness × Low-Missingness Items

If the ~60% missingness on `democracy_suitability` were driven by political
self-censorship rather than the instrument's question sequence, respondents
missing on suitability should show *different* attitudinal patterns on
low-missingness items compared to those who answered. This section tests that
prediction directly.

```{r suitability-missingness-crosstab}
# Create indicator for whether respondent answered democracy_suitability
hk5_analysis <- hk5_analysis |>
  mutate(suit_responded = !is.na(democracy_suitability))

# For each low-missing item, compare means between those who answered
# democracy_suitability and those who did not, within each period
low_miss_items <- c("dem_vs_econ", "dem_vs_equality")
low_miss_items <- low_miss_items[low_miss_items %in% names(hk5_analysis)]

crosstab_results <- map_dfr(low_miss_items, function(var) {
  map_dfr(c("Protest", "Post-NSL"), function(per) {
    sub <- hk5_analysis |>
      filter(period == per, !is.na(!!sym(var)))

    responded <- sub |> filter(suit_responded) |> pull(!!sym(var))
    missing   <- sub |> filter(!suit_responded) |> pull(!!sym(var))

    if (length(responded) < 5 | length(missing) < 5) return(NULL)

    tt <- t.test(responded, missing)
    pooled_sd <- sqrt(((length(responded) - 1) * sd(responded)^2 +
                         (length(missing) - 1) * sd(missing)^2) /
                        (length(responded) + length(missing) - 2))
    d <- (mean(missing) - mean(responded)) / pooled_sd

    tibble(
      variable = var,
      period = per,
      n_suit_responded = length(responded),
      mean_suit_responded = mean(responded),
      n_suit_missing = length(missing),
      mean_suit_missing = mean(missing),
      diff = mean(missing) - mean(responded),
      cohens_d = d,
      p_value = tt$p.value
    )
  })
...
crosstab_results |>
  mutate(across(c(mean_suit_responded, mean_suit_missing, diff, cohens_d), ~round(.x, 3)),
         p_value = format.pval(p_value, digits = 3)) |>
  kable(
    col.names = c("Variable", "Period",
                   "N (answered suit.)", "Mean (answered suit.)",
                   "N (missing suit.)", "Mean (missing suit.)",
                   "Diff", "Cohen's d", "p-value"),
    caption = paste0(
      "Attitudinal patterns on low-missingness democracy items, by whether respondent ",
      "answered democracy_suitability. Small or non-significant differences suggest ",
      "suitability missingness is instrumentally rather than politically driven."
    ),
    booktabs = TRUE
  ) |>
  kable_styling(latex_options = c("scale_down", "hold_position"))

# Logistic regression: does suitability missingness predict low-miss item values?
if (length(low_miss_items) > 0) {
  cat("\n=== Logistic regression: suitability non-response ~ low-miss item values ===\n")
  for (var in low_miss_items) {
    for (per in c("Protest", "Post-NSL")) {
      sub <- hk5_analysis |> filter(period == per, !is.na(!!sym(var)))
      if (nrow(sub) > 30) {
        mod <- glm(suit_responded ~ ., data = sub |> select(suit_responded, !!sym(var)),
                   family = binomial)
        cat(sprintf("\n%s (%s):\n", var, per))
...
# 8. Survey-Weighted Estimates

The main analysis uses unweighted estimates because the ABS sampling weights
are designed for the full Hong Kong sample, not for the within-wave sub-period
comparison that constitutes the primary analysis. However, Reviewer 2 correctly
notes that "available upon request" is insufficient. This section reproduces
the core results using the ABS design weights (`w` in the raw SPSS file) to
demonstrate that weighted and unweighted estimates are substantively identical.

```{r load-weights}
# Read weight variable from raw ABS Wave 5 SPSS file
library(haven)
source("../../../_data_config.R")
raw_w5 <- read_sav(file.path(survey_data_prep, "data/abs/raw/wave5/20230505_W5_merge_15.sav"),
                   col_select = c("COUNTRY", "IDnumber", "w"))

# Filter to Hong Kong (country code 2)
hk_weights <- raw_w5 |>
  filter(COUNTRY == 2) |>
  select(IDnumber, abs_weight = w) |>
  mutate(IDnumber = as.numeric(IDnumber))

cat("HK weight records:", nrow(hk_weights), "\n")
cat("Weight range:", round(range(hk_weights$abs_weight), 4), "\n")
cat("Weight mean:", round(mean(hk_weights$abs_weight), 4), "\n")

# Merge weights into analysis dataset
hk5_weighted <- hk5_analysis |>
  mutate(IDnumber = as.numeric(as.character(idnumber))) |>
  left_join(hk_weights, by = "IDnumber")

cat("Matched:", sum(!is.na(hk5_weighted$abs_weight)), "of", nrow(hk5_weighted), "\n")

# If merge fails on IDnumber, try positional matching by row order
if (sum(!is.na(hk5_weighted$abs_weight)) < nrow(hk5_weighted) * 0.5) {
  cat("WARNING: IDnumber merge matched <50%. Attempting row-order merge for HK W5.\n")
  hk_w5_raw <- raw_w5 |> filter(COUNTRY == 2)
  if (nrow(hk_w5_raw) == nrow(hk5_analysis) ||
      nrow(hk_w5_raw) >= nrow(hk5_analysis)) {
    # The prepared data is a subset of W5 HK; try matching by idnumber variants
    hk5_weighted <- hk5_analysis |>
      mutate(abs_weight = NA_real_)
    cat("Row-order merge not feasible without matching key. Using uniform weights.\n")
  }
}
```

```{r weighted-estimates}
# Reproduce core results with ABS design weights
# Use the same variables as the main NSL comparison

if (sum(!is.na(hk5_weighted$abs_weight)) > 100) {
  weighted_results <- map_dfr(nsl_vars, function(var) {
    sub <- hk5_weighted |>
      filter(!is.na(!!sym(var)), !is.na(abs_weight))

    protest <- sub |> filter(period == "Protest")
    postnsl <- sub |> filter(period == "Post-NSL")

    if (nrow(protest) < 10 | nrow(postnsl) < 10) return(NULL)

    # Weighted means
    wt_mean_protest <- weighted.mean(protest[[var]], protest$abs_weight)
    wt_mean_postnsl <- weighted.mean(postnsl[[var]], postnsl$abs_weight)

    # Unweighted means for comparison
    unwt_mean_protest <- mean(protest[[var]])
    unwt_mean_postnsl <- mean(postnsl[[var]])

    # Weighted t-test using survey package
    if (requireNamespace("survey", quietly = TRUE)) {
      des <- survey::svydesign(ids = ~1, weights = ~abs_weight, data = sub)
      tt <- survey::svyttest(as.formula(paste(var, "~ period")), design = des)
      wt_p <- tt$p.value
    } else {
      # Fallback: WLS regression
      mod <- lm(as.formula(paste(var, "~ period")), data = sub, weights = abs_weight)
      wt_p <- summary(mod)$coefficients["periodPost-NSL", "Pr(>|t|)"]
    }

    # Unweighted t-test
    unwt_tt <- t.test(protest[[var]], postnsl[[var]])

    # Cohen's d (unweighted, for comparability with main text)
    pooled_sd <- sqrt(((nrow(protest) - 1) * sd(protest[[var]])^2 +
                         (nrow(postnsl) - 1) * sd(postnsl[[var]])^2) /
                        (nrow(protest) + nrow(postnsl) - 2))
    d <- (mean(postnsl[[var]]) - mean(protest[[var]])) / pooled_sd

    tibble(
      variable = var,
      unwt_protest = unwt_mean_protest,
      unwt_postnsl = unwt_mean_postnsl,
      unwt_d = d,
      unwt_p = unwt_tt$p.value,
      wt_protest = wt_mean_protest,
      wt_postnsl = wt_mean_postnsl,
      wt_diff = wt_mean_postnsl - wt_mean_protest,
      wt_p = wt_p,
      direction_match = sign(wt_mean_postnsl - wt_mean_protest) ==
                        sign(unwt_mean_postnsl - unwt_mean_protest)
    )
  })

  # Display comparison table
  weighted_results |>
    mutate(across(c(unwt_protest, unwt_postnsl, unwt_d,
                    wt_protest, wt_postnsl, wt_diff), ~round(.x, 3)),
           unwt_p = format.pval(unwt_p, digits = 3),
           wt_p = format.pval(wt_p, digits = 3)) |>
    kable(
      col.names = c("Variable",
                     "Unwt Protest", "Unwt Post-NSL", "Unwt d", "Unwt p",
                     "Wt Protest", "Wt Post-NSL", "Wt Diff", "Wt p",
                     "Same dir?"),
      caption = paste0(
        "Unweighted vs. ABS design-weighted estimates for all NSL comparison variables. ",
        "Direction match: ", sum(weighted_results$direction_match, na.rm = TRUE),
        " of ", nrow(weighted_results), " variables."
      ),
      booktabs = TRUE
    ) |>
    kable_styling(latex_options = c("scale_down", "hold_position"))
  ...
  # Summary
  cat("\nDirection agreement:",
      sum(weighted_results$direction_match, na.rm = TRUE), "of",
      nrow(weighted_results), "variables\n")

  # Focus on primary outcomes
  primary <- c("trust_police", "trust_national_government",
               "democracy_suitability", "gov_free_to_organize")
  cat("\nPrimary outcomes (weighted vs unweighted):\n")
  weighted_results |>
    filter(variable %in% primary) |>
    select(variable, unwt_d, unwt_p, wt_diff, wt_p) |>
    mutate(across(c(unwt_d, wt_diff), ~round(.x, 3)),
           unwt_p = format.pval(unwt_p, digits = 3),
           wt_p = format.pval(wt_p, digits = 3)) |>
    print()

} else {
  cat("\nWARNING: Insufficient weight matches. Cannot produce weighted estimates.\n")
  cat("The ABS harmonized dataset does not include the weight variable 'w'.\n")
  cat("To resolve: add 'w' from the raw SPSS file to the data preparation pipeline.\n")
...
save(mw_results, balance_table, boot_results, manski_scenarios,
     ...
     mnar_suitability_post_to_max,
     convergent_results, crosstab_results,
     weighted_results,
