...
cont_results <- map_dfr(continuous_vars, function(var) {
  ...
  tibble(
    ...
    protest_sd = sd(protest),
    postnsl_mean = mean(postnsl),
    postnsl_sd = sd(postnsl),
    cohens_d = d,
    t_stat = tt$statistic,
    t_p = tt$p.value,
...
proc_by_period <- dem_ess_data |>
  ...
  group_by(period) |>
  summarise(pct_procedural = mean(procedural) * 100, .groups = "drop")
cat("\nProcedural (electoral + expressive) share:\n")
...
# Table labels (no newlines for kable)
dem_ess_labels_table <- c(
  `1` = "Electoral (elections)",
  `2` = "Expressive (free expression)",
  `4` = "Basic Needs (necessities)",
  `5` = "Good Governance (clean politics)"
...
ggplot(dem_ess_plot, aes(x = dem_label, y = prop, fill = period)) +
  ...
  labs(
    ...
    caption = "Source: ABS W5 Hong Kong. Labels: 1=Electoral, 2=Expressive, 4=Basic Needs, 5=Good Governance.\nNote: Redistributive (3) not available in W5."
  ) +
...
china_means <- china_data |>
  group_by(period) |>
  summarise(mean_val = mean(dem_rating_china), .groups = "drop")
...
demo_means <- demo_data |>
  group_by(period) |>
  summarise(mean_val = mean(action_demonstration), .groups = "drop")
...
demo_bar <- demo_data |>
  ...
  ungroup() |>
  mutate(
    action_label = recode(action_demonstration,
      `1` = "Would\nnever do",
      `2` = "Not done,\nbut might",
      `3` = "Done\nonce",
      `4` = "Done\n2-3x",
...
demo_age <- hk5_analysis |>
  ...
  group_by(period, age_group) |>
  summarise(
    mean = mean(action_demonstration),
    se = sd(action_demonstration) / sqrt(n()),
    n = n(),
    .groups = "drop"
  )
...
china_age <- hk5_analysis |>
  ...
  group_by(period, age_group) |>
  summarise(
    mean = mean(dem_rating_china),
    se = sd(dem_rating_china) / sqrt(n()),
    n = n(),
    .groups = "drop"
  )
