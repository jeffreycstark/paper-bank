...
```{r manski-bounds}
# We don't know true non-response rate, but we can simulate scenarios
# Assume various proportions of "missing critics" in post-NSL period

trust_police_protest <- hk5_analysis |>
  filter(period == "Protest", !is.na(trust_police)) |>
  pull(trust_police)

trust_police_postnsl <- hk5_analysis |>
  filter(period == "Post-NSL", !is.na(trust_police)) |>
  pull(trust_police)

observed_postnsl_mean <- mean(trust_police_postnsl)
observed_protest_mean <- mean(trust_police_protest)

# Scenario: what if X% additional respondents would have participated
# but reported minimum trust (1 on 1-4 scale)?
manski_scenarios <- tibble(
  pct_missing_critics = seq(0, 30, by = 5)
) |>
  mutate(
    n_observed = length(trust_police_postnsl),
    n_missing = round(n_observed * pct_missing_critics / 100),
    adjusted_mean = (sum(trust_police_postnsl) + n_missing * 1) / (n_observed + n_missing),
...
manski_scenarios |>
  ...
  kable(
    ...
    caption = paste0("Manski-style worst-case bounds for trust in police. ",
                     "Protest mean = ", round(observed_protest_mean, 3),
                     "; Observed post-NSL mean = ", round(observed_postnsl_mean, 3)),
    booktabs = TRUE
  ) |>
  kable_styling(latex_options = c("hold_position"))

cat("\nInterpretation: The trust increase disappears only if approximately",
    manski_scenarios |> filter(!trust_still_higher) |> slice_min(pct_missing_critics) |> pull(pct_missing_critics),
    "% or more of the post-NSL target population declined to participate\n",
    "and would have reported minimum trust (1 on 1-4 scale).\n")
```
