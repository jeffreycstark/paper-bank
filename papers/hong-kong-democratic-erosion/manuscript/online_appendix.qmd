...
, digits = 2, eps = .001)
  ...
  cat("\n**Panel A: Omnibus chi-squared tests (3 categories × response/non-response)**\n\n")
  print(
    omnibus |>
  ...
  # Panel B: Pairwise + odds ratios
  pairwise <- bind_rows(
    wvs_gradient_respondent_tests$Turkey$pairwise,
    wvs_gradient_respondent_tests$Russia$pairwise
  ) |>
    rename(Country = country, Comparison = comparison,
           `$\chi^2---
title: "Online Appendix: When the Levee Breaks"
author:
  - name: Jeffrey Stark
...
, digits = 2, eps = .001)
  ...
  or_tbl <- wvs_gradient_odds_ratios |>
    mutate(
      OR = sprintf("%.2f [%.2f, %.2f]", odds_ratio, or_ci_lo, or_ci_hi)
    ) |>
    select(country, OR) |>
    rename(Country = country)

  cat("\n**Panel B: Pairwise comparisons and odds ratios (Democracy vs. Trust)**\n\n")
  print(
    pairwise |>
      kable(booktabs = TRUE, escape = FALSE) |>
      kable_styling(latex_options = c("hold_position"))
  ...
  cat("\n")
  cat("\nOdds of non-response, Democracy items relative to Trust items:\n\n")
  print(
    or_tbl |>
      kable(booktabs = TRUE, col.names = c("Country", "OR [95% CI]")) |>
      kable_styling(latex_options = c("hold_position")) |>
      footnote(
        general = "OR > 1 indicates democracy items have higher odds of non-response than trust items. CIs computed via Wald method on the log-odds scale.",
        general_title = "Note: ",
        footnote_as_chunk = TRUE
      )
  )
...
In both Turkey and Russia, democracy-related items exhibit substantially higher non-response rates in Wave 7 than institutional confidence items, which in turn exceed apolitical control items. In Turkey, the average Wave 7 non-response rate for democracy items (excluding the army rule item, which was not fielded) is approximately 7% compared to 1.4% for trust items and 0.4% for control items. In Russia, the corresponding figures are approximately 12%, 4.5%, and 1.3%. This monotonic gradient---democracy items > trust items > control items---parallels the differential item non-response documented in the Hong Kong ABS data (Online Appendix F), where normative democracy items exhibited a 4.6 percentage point decline in response rates post-NSL while trust items remained stable. The underlying mechanism is the same: respondents selectively avoid items where honest answers carry the highest political risk.

Notably, Turkey's "army rule" item (Q237) was entirely absent from the Wave 7 questionnaire, constituting the most extreme form of item suppression: the survey organization itself removed the most politically sensitive question in the post-coup context. This absence underscores the practical relevance of the sensitivity gradient for survey practitioners working in autocratizing environments---and represents a more severe version of the item non-response pattern, in which the gatekeeping function shifts from individual respondents to the survey organization itself.

The trust paradox is also visible in both cases, mirroring the core finding from Hong Kong where trust in police surged ($d \approx +0.49$) while democracy suitability collapsed ($d = -0.54$). Despite substantial democratic backsliding as measured by V-Dem indices, confidence in state institutions either increased or remained stable between WVS waves in both Turkey and Russia. In Turkey, confidence in government, police, courts, and armed forces all shifted toward greater trust---the same V-shaped recovery pattern observed for coercive institutions in the Hong Kong ABS data, where post-NSL trust means reverted to pre-protest WVS baselines. In Russia, the trust pattern holds, with confidence in parliament showing the largest increase. Meanwhile, Russia's democracy evaluation score *increased* by over one full point on the 1--10 scale---a pattern more consistent with rally-around-the-flag effects and preference falsification than with genuine improvements in democratic governance. This Russian case represents an even more extreme version of the trust paradox than Hong Kong: whereas Hong Kong's democratic evaluations at least moved in the expected direction (declining), Russia's moved *counter* to observable autocratization, suggesting that the falsification pressure was strong enough to distort not only trust items but evaluative items as well.
...
These findings demonstrate that the sensitivity gradient is not idiosyncratic to the Hong Kong context or the Asian Barometer Survey instrument. The same pattern---systematic differential non-response scaling with item political sensitivity---emerges independently in WVS data from two additional autocratizing contexts with distinct political histories, repressive mechanisms, and survey implementations. Three features of this cross-national evidence are worth emphasizing. First, the gradient emerges across different survey programs (ABS vs. WVS), different languages, different fieldwork organizations, and different interview modes, ruling out instrument-specific artifacts. Second, the gradient manifests in contexts with different types of repression---Hong Kong's rapid, externally imposed NSL, Turkey's post-coup emergency powers, and Russia's incremental civil society squeeze---suggesting the mechanism operates across the spectrum of autocratization pathways. Third, the apolitical control items (happiness, life satisfaction, health) show negligible non-response in all three cases, confirming that the elevated non-response on political items is not driven by generalized survey fatigue or fieldwork quality issues but by the political content of the items themselves.

Taken together with the Hong Kong evidence, these results support the interpretation that the sensitivity gradient constitutes a generalizable diagnostic for survey validity under political pressure. Researchers encountering the trust paradox---rising institutional confidence coexisting with declining democratic evaluations---in survey data from autocratizing contexts should treat the pattern as a prima facie indicator of measurement distortion warranting further investigation rather than evidence of genuine regime legitimacy.
