...
This framework is best suited to contexts where autocratization is rapid and survey fieldwork spans the transition. While the Hong Kong case provides unusual identification leverage through within-wave fieldwork disruption, the diagnostic patterns generalize to cross-wave comparisons in other settings, as I demonstrate with World Values Survey data from Turkey (Section 4.5). The approach requires only standard trust and democratic evaluation items available in all major cross-national survey programs.

These findings contribute to three literatures. First, they refine the measurement of democratic backsliding [@Waldner2018-yw], offering a diagnostic tool---replicated cross-nationally in Turkey---for researchers working in closing spaces. Second, they engage debates on authoritarian legitimacy [@Nathan2003-wg; @Tang2016-nl; @Dukalskis2017-md], providing empirical grounds to question whether "performance legitimacy" in autocracies is often an artifact of survey non-response and falsification. Finally, they bridge the gap between critical theory and survey methodology, operationalizing the concern that quantitative metrics can inadvertently reproduce the logic of domination by masking the silence of the marginalized.
...
## Cross-National Evidence: Turkey

A natural question is whether the sensitivity gradient is specific to Hong Kong's particular context or whether it captures a more general pattern of survey distortion under autocratization. To assess this, I apply the same framework to Turkey using World Values Survey data spanning the 2016 failed coup attempt---one of the most dramatic autocratization episodes in recent comparative politics. Following the coup attempt, the Erdoğan government declared a state of emergency lasting two years, purged over 150,000 civil servants, detained more than 50,000 individuals, shuttered over 150 media outlets, and enacted constitutional changes consolidating executive power [@Esen2016-iq; @Somer2016-tj]. Turkey's V-Dem liberal democracy index fell from 0.35 in 2013 to 0.11 by 2019, among the steepest declines recorded globally. WVS Wave 6 (fielded in Turkey in 2011) and Wave 7 (fielded in 2018) thus bracket this period of rapid autocratization, providing a direct analogue to the Hong Kong pre/post NSL comparison.
...
```{r}
#| label: fig-turkey-gradient
#| fig-cap: "Sensitivity gradient in Turkey: Cohen's *d* (WVS Wave 7 $-$ Wave 6) by item, ordered by pre-specified sensitivity to preference falsification. Institutional confidence items (red) shift regime-favorable; democratic evaluation items (blue) shift regime-unfavorable or remain stable. Dashed line: OLS fit."
#| fig-width: 6.5
#| fig-height: 4.5
...
if (file.exists("../analysis/results/turkey_gradient.RData")) {
  ...
  plot_data <- turkey_gradient_results |>
    filter(label != "Army rule") |>
    mutate(
      plot_label = case_when(
        label == "Conf. police" ~ "Conf. Police",
        label == "Conf. government" ~ "Conf. Government",
        label == "Conf. armed forces" ~ "Conf. Armed Forces",
        label == "Conf. parliament" ~ "Conf. Parliament",
        label == "Conf. pol. parties" ~ "Conf. Pol. Parties",
        label == "Conf. courts" ~ "Conf. Courts",
        label == "Conf. press" ~ "Conf. Press",
        label == "Dem. importance" ~ "Dem. Importance (1-10)",
        label == "Dem. evaluation" ~ "Dem. Evaluation (1-10)",
        label == "Democratic system" ~ "Dem. System Good",
        label == "Strong leader" ~ "Strong Leader",
        label == "Experts decide" ~ "Experts Decide",
        TRUE ~ label
      ),
      category = factor(category, levels = c("High", "Medium", "Low")),
      ...
      ci_lo = cohens_d - 1.96 * se_d,
      ci_hi = cohens_d + 1.96 * se_d,
      plot_label = factor(plot_label, levels = rev(plot_label[order(sensitivity_rank)]))
    )

  fit <- lm(cohens_d ~ sensitivity_rank, data = plot_data)
  plot_data$fitted <- predict(fit)

  cat_colors <- c(High = "#C62828", Medium = "#F57C00", Low = "#1565C0")
  cat_shapes <- c(High = 16, Medium = 15, Low = 17)

  ggplot(plot_data, aes(x = cohens_d, y = plot_label)) +
    geom_vline(xintercept = 0, linetype = "solid", color = "grey70", linewidth = 0.4) +
    geom_line(aes(x = fitted, y = plot_label, group = 1),
              linetype = "dashed", color = "grey50", linewidth = 0.5) +
    geom_errorbarh(aes(xmin = ci_lo, xmax = ci_hi, color = category),
                   height = 0.2, linewidth = 0.4) +
    geom_point(aes(color = category, shape = category), size = 2.5) +
    scale_color_manual(values = cat_colors, name = "Sensitivity") +
    scale_shape_manual(values = cat_shapes, name = "Sensitivity") +
    labs(x = "Cohen's *d* (Wave 7 minus Wave 6)", y = NULL) +
    theme_minimal(base_size = 11) +
    theme(
      axis.title.x = ggtext::element_markdown(),
      legend.position = "bottom",
...
@tbl-turkey-gradient and @fig-turkey-gradient present the results. The pattern closely mirrors Hong Kong. Confidence in coercive institutions uniformly increased between 2011 and 2018: police ($d = +0.21$), government ($d = +0.23$), and armed forces ($d = +0.24$) all show statistically significant regime-favorable shifts. Meanwhile, the importance of democracy---the most abstract evaluative item---declined sharply ($d = -0.36$), and approval of a democratic political system fell significantly ($d = -0.24$). The gradient correlation across all items ($r = -0.70$) is consistent with the prediction that the magnitude and direction of observed shifts track pre-specified sensitivity rankings, paralleling the Hong Kong pattern ($r = -0.85$).

The Turkey case also reveals instructive variation. Confidence in political parties shows an anomalously large positive shift ($d = +0.36$), likely reflecting the AKP's genuine consolidation of partisan support alongside measurement distortion---a reminder that the sensitivity gradient identifies the *pattern* of distortion rather than claiming all measured change is artificial. Control items (happiness, life satisfaction) moved in the opposite direction from trust ($d = -0.15$ and $d = -0.33$, respectively), weighing against a generalized positivity bias and strengthening the inference that trust increases are domain-specific. The cross-national replication, using a different survey instrument (WVS rather than ABS), different item wordings, and a distinct form of autocratization (post-coup executive consolidation rather than externally imposed security legislation), substantially strengthens the case that the sensitivity gradient constitutes a generalizable diagnostic rather than a Hong Kong-specific artifact. Full item-level results with non-response analysis are reported in Online Appendix H.
...
```{r}
#| label: tbl-toolkit
#| tbl-cap: "Diagnostic checklist for survey distortion in autocratizing contexts. HK = Hong Kong (ABS W5), TR = Turkey (WVS W6--W7), RU = Russia (WVS W6--W7)."
...
toolkit |>
  ...
  column_spec(4:6, width = "0.8cm") |>
  footnote(
    symbol = c(
      "\\\\checkmark = strong evidence; \\\\texttildelow = suggestive; ? = data limitations; -- = not tested."
    ),
    general = c(
...
The sensitivity gradient approach does not resolve every measurement challenge in authoritarian survey research. It works best when repression is uneven, political change is rapid, and surveys contain items spanning a range of political sensitivity. While proof-of-concept relies on Hong Kong's unique natural experiment, preliminary evidence from Turkey (Section 4.5) and Russia (Online Appendix H) suggests the core diagnostic patterns---differential item inflation and correlation reversals---appear across autocratization contexts with distinct political histories and survey instruments. For the growing number of cases where these conditions obtain, the sensitivity gradient offers an immediately applicable tool for assessing whether the data can be trusted to mean what it appears to say.
